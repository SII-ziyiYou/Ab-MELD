...
def main(args):
    ...
    #config = AutoConfig.from_pretrained('PLM/prot_t5_xl_uniref50')
    #assert args.tx_dim%ATTN_HEAD_DIM==0, f'Transformer dimension must be divisible by {ATTN_HEAD_DIM}'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default='All_seqs')
    ...
    parser.add_argument("--output_dir", type=str, default='Generate_dir')
    ...
    parser.add_argument("--enc_dec_model", type=str, default="PLM/prot_t5_xl_uniref50")
    ...
    parser.add_argument("--latent_model_path", type=str, default='latent_model_Anti_32_64')
    ...
    if args.config:
        ...
        # Handle cases where CLI might override config
        # We re-parse to identify which arguments were explicitly provided on CLI
        # But for simplicity, we can just update the ones that are in config_args
        # and haven't been changed from their defaults.
        # A more robust way is to parse again.
        conf_parser = argparse.ArgumentParser(description="Training arguments", argument_default=argparse.SUPPRESS)
        # Copy all arguments to the new parser
        for action in parser._actions:
            if action.dest == 'config': continue
            conf_parser.add_argument(action.option_strings[0], type=action.type, help=action.help)
        
        # Parse again to see what was actually on CLI
        cli_args = parser.parse_args()
        args_dict = vars(cli_args)
        
        # Update with config values
        for k, v in config_args.items():
            if k not in args_dict or parser.get_default(k) == args_dict[k]:
                args_dict[k] = v
    ...
    if args.eval or args.resume_training or args.eval_test:
        ...
        heldout_params = { 'output_dir', 'resume_dir', 'eval', 'eval_test', 'num_samples', 'sampling_timesteps', 'sampling_schedule', 'scale', 'sampler', 'resume_training'}
